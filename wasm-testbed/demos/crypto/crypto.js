// Cryptography Benchmarks - WASM vs JavaScript

let jsHashTime = 0;
let wasmHashTime = 0;

document.addEventListener('DOMContentLoaded', () => {
    const runSha256JsBtn = document.getElementById('runSha256Js');
    const runSha256WasmBtn = document.getElementById('runSha256Wasm');

    if (runSha256JsBtn) {
        runSha256JsBtn.addEventListener('click', () => runHashBenchmark('js'));
    }

    if (runSha256WasmBtn) {
        runSha256WasmBtn.addEventListener('click', () => runHashBenchmark('wasm'));
    }
});

async function runHashBenchmark(mode) {
    const dataSize = document.getElementById('dataSize').value;
    const iterations = parseInt(document.getElementById('iterations').value);
    const input = document.getElementById('cryptoInput').value;

    // Generate data based on size
    let data;
    switch (dataSize) {
        case 'small':
            data = input || 'The quick brown fox jumps over the lazy dog';
            break;
        case 'medium':
            data = generateRandomText(10); // 10 KB
            break;
        case 'large':
            data = generateRandomText(1024); // 1 MB
            break;
        case 'huge':
            data = generateRandomText(10240); // 10 MB
            break;
    }

    const encoder = new TextEncoder();
    const dataBuffer = encoder.encode(data);

    let hash;
    let totalTime = 0;

    if (mode === 'js') {
        // JavaScript SHA-256 implementation
        const start = performance.now();

        for (let i = 0; i < iterations; i++) {
            hash = await sha256JS(dataBuffer);
        }

        const end = performance.now();
        totalTime = end - start;
        jsHashTime = totalTime;

        document.getElementById('jsHashTime').textContent = formatTime(totalTime);
        document.getElementById('hashOutput').textContent = hash;
    } else {
        // WebAssembly SHA-256 (fallback to Web Crypto API for now)
        const start = performance.now();

        for (let i = 0; i < iterations; i++) {
            const hashBuffer = await crypto.subtle.digest('SHA-256', dataBuffer);
            hash = Array.from(new Uint8Array(hashBuffer))
                .map(b => b.toString(16).padStart(2, '0'))
                .join('');
        }

        const end = performance.now();
        totalTime = end - start;
        wasmHashTime = totalTime;

        document.getElementById('wasmHashTime').textContent = formatTime(totalTime) + ' (Native Crypto API)';
        document.getElementById('hashOutput').textContent = hash;
    }

    updateHashSpeedup();
    updateCryptoChart();
}

function updateHashSpeedup() {
    if (jsHashTime > 0 && wasmHashTime > 0) {
        const speedup = (jsHashTime / wasmHashTime).toFixed(2);
        document.getElementById('hashSpeedup').textContent = `${speedup}x faster`;
    }
}

function updateCryptoChart() {
    const canvas = document.getElementById('cryptoChart');
    if (!canvas) return;

    if (jsHashTime > 0 && wasmHashTime > 0) {
        drawBarChart(
            canvas,
            [jsHashTime, wasmHashTime],
            ['JavaScript', 'Native API'],
            ['#f7df1e', '#654ff0']
        );
    }
}

// Pure JavaScript SHA-256 implementation
async function sha256JS(data) {
    // Convert to array if needed
    const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);

    // Initialize hash values (first 32 bits of fractional parts of square roots of first 8 primes)
    let h0 = 0x6a09e667;
    let h1 = 0xbb67ae85;
    let h2 = 0x3c6ef372;
    let h3 = 0xa54ff53a;
    let h4 = 0x510e527f;
    let h5 = 0x9b05688c;
    let h6 = 0x1f83d9ab;
    let h7 = 0x5be0cd19;

    // Initialize array of round constants (first 32 bits of fractional parts of cube roots of first 64 primes)
    const k = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ];

    // Pre-processing
    const ml = bytes.length * 8;
    const paddedLength = Math.ceil((ml + 65) / 512) * 64;
    const padded = new Uint8Array(paddedLength);
    padded.set(bytes);
    padded[bytes.length] = 0x80;

    // Append length as 64-bit big-endian
    const view = new DataView(padded.buffer);
    view.setUint32(paddedLength - 4, ml & 0xffffffff, false);
    view.setUint32(paddedLength - 8, Math.floor(ml / 0x100000000), false);

    // Process each 512-bit chunk
    for (let chunkStart = 0; chunkStart < paddedLength; chunkStart += 64) {
        const w = new Uint32Array(64);

        // Copy chunk into first 16 words
        for (let i = 0; i < 16; i++) {
            w[i] = view.getUint32(chunkStart + i * 4, false);
        }

        // Extend the first 16 words into remaining 48 words
        for (let i = 16; i < 64; i++) {
            const s0 = rightRotate(w[i - 15], 7) ^ rightRotate(w[i - 15], 18) ^ (w[i - 15] >>> 3);
            const s1 = rightRotate(w[i - 2], 17) ^ rightRotate(w[i - 2], 19) ^ (w[i - 2] >>> 10);
            w[i] = (w[i - 16] + s0 + w[i - 7] + s1) >>> 0;
        }

        // Initialize working variables
        let a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;

        // Main loop
        for (let i = 0; i < 64; i++) {
            const S1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
            const ch = (e & f) ^ (~e & g);
            const temp1 = (h + S1 + ch + k[i] + w[i]) >>> 0;
            const S0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
            const maj = (a & b) ^ (a & c) ^ (b & c);
            const temp2 = (S0 + maj) >>> 0;

            h = g;
            g = f;
            f = e;
            e = (d + temp1) >>> 0;
            d = c;
            c = b;
            b = a;
            a = (temp1 + temp2) >>> 0;
        }

        // Add compressed chunk to current hash value
        h0 = (h0 + a) >>> 0;
        h1 = (h1 + b) >>> 0;
        h2 = (h2 + c) >>> 0;
        h3 = (h3 + d) >>> 0;
        h4 = (h4 + e) >>> 0;
        h5 = (h5 + f) >>> 0;
        h6 = (h6 + g) >>> 0;
        h7 = (h7 + h) >>> 0;
    }

    // Produce final hash value
    const hash = [h0, h1, h2, h3, h4, h5, h6, h7]
        .map(h => h.toString(16).padStart(8, '0'))
        .join('');

    return hash;
}

function rightRotate(value, amount) {
    return (value >>> amount) | (value << (32 - amount));
}
